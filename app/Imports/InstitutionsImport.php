...
class InstitutionsImport
{
    ...
    private function processRow($row, $rowNumber)
    {
        ...
        if (!empty($row[5]) && $province) {
            $municipality = Municipality::where('name', trim($row[5]))
        ...
        if (!empty($row[6]) && $municipality) {
            $locality = Locality::where('name', trim($row[6]))
        ...
        if (!empty($row[7]) && $municipality) {
            $district = District::where('name', trim($row[7]))
                               ->where('municipality_id', $municipality->id)
                               ->first();
        }
        $zone = null;
        if (!empty($row[8])) {
            $zone = Zone::where('name', trim($row[8]))->first();
        }
        $active = true;
        if (isset($row[13])) {
